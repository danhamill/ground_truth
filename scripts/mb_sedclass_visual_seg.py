...
if __name__ == '__main__':
    ...
    fig, (ax,ax1) = plt.subplots(figsize=(12,10),ncols=2)
    
    #Start with april sidescan sonar
    ax.set_title('April 2014 \n Sidescan Sonar Imagery')
    m = Basemap(projection='merc', 
                epsg=cs2cs_args.split(':')[1], 
                llcrnrlon=np.nanmin(april_lon)-0.0009, 
                llcrnrlat=np.nanmin(april_lat)-0.0006,
                urcrnrlon=np.nanmax(april_lon)+0.0009, 
                urcrnrlat=np.nanmax(april_lat)+0.0006,ax=ax)
    ...
    ax1.legend((circ5, circ3,circ1),('Sand','Gravel','Gravel'),numpoints=1, loc='best')
    plt.tight_layout()
    plt.show()
