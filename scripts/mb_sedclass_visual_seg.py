...
if __name__ == '__main__':
    ...
    #Load May 2014 Data
    may_sed_class_raster = r"C:\workspace\Reach_4a\Multibeam\mb_sed_class\may2014_mb6086r_sedclass\mb_sed3class_2014_05_raster.tif"
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig(root + os.sep + 'misaligned_ss_imagery_example_lowres.png',dpi=200)
...
#########################################################################################################################################
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig(root + os.sep + 'mb_ss_ground_truth_area_lowres.png',dpi=200)
    ...
    plt.tight_layout()
    plt.savefig(root + os.sep + 'sed3class_distributions.png')
    ...
    ds = gdal.Open(r"C:\workspace\Reach_4a\Multibeam\mb_sed_class\input\may_14_mbsed5class.tif")
    may_sed_class = ds.GetRasterBand(1).ReadAsArray()
    may_sed_class[may_sed_class<0]=np.nan
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig(root + os.sep + 'mb_5_Class_ground_truth_area_lowres.png',dpi=200)
    ...
    ax.set_xlabel('Substrate Type')
    plt.tight_layout()
    plt.savefig(root + os.sep + 'sed5class_distributions.png')
    
    
    
    cmap = {1.0: 'Sand',2.0:'Sand_Gravel', 3.0: 'Gravel',4.0:'Sand_Rock', 5.0: 'Boulders'}
    stats = zonal_stats(seg_shp,r"C:\workspace\Reach_4a\Multibeam\mb_sed_class\input\may_14_mbsed5class.tif",categorical=True,nodata=-99,category_map = cmap)
    
    merge_df = pd.DataFrame(stats)
    
    merge_df['Gravel2'] = merge_df['Sand_Gravel'] + merge_df['Sand_Rock'] +merge_df['Gravel']
    merge_df['Substrate'] = get_subs(seg_shp)
    merge_df = merge_df[['Sand','Gravel2','Boulders','Substrate']]
